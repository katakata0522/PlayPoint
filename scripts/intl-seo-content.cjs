...
const LOCALES = {
  ...
  tw: {
    ...
    estimateBody: '計算結果可用來把原本規劃的購買與帳號確認適用的積點率相配合。請在 Google Play App 確認適用商品與活動期間，點數入帳後再計算剩餘進度。',
...
const PAGE_TYPES = {
  ...
  platinum: {
    ...
    tw: {
      ...
      summary: ['預設: 黃金級到白金', '適合: 規劃下一個等級', '確認: 剩餘點數、特別獲點率、地區條件'],
      sections: [
  ...
  gold: {
    ...
    tw: {
      title: 'Google Play Points 黃金級需要多少？',
      description: '估算 Google Play Points 黃金級所需點數與金額，並用目前等級、剩餘點數與活動特別獲點率比較是否值得升級。',
      eyebrow: '黃金級規劃',
      h1: 'Google Play Points 黃金級需要多少？',
      lead: '黃金級通常是第一個比較容易規劃的目標。請輸入還差多少點，再比較一般回饋、活動特別獲點率與地區條件。',
      cta: '計算黃金級進度',
      secondary: '比較白金等級',
      ...
      faq: [
        ['黃金級適合作為第一個目標嗎？', '通常適合，因為比白金或鑽石更容易用平常消費來比較。'],
  ...
  silver: {
    ...
    tw: {
      ...
      lead: '台灣銀級從年度 250 點開始。先確認還差多少點，再把原本規劃的購買安排到帳號顯示的最高適用積點率。',
      cta: '計算台灣銀級進度',
      secondary: '查看台灣銀級攻略',
      ...
      sections: [
        ...
        ['比較最高適用積點率', '先看每 NT$30 1 點，再比較已啟用且符合原定購買的優惠。多個促銷不會疊加。'],
        ['確認商品別結果', 'Google 以未稅商品價格計算，每件商品四捨五入到最接近的整數點。']
      ],
      faq: [
        ['台灣銀級需要幾點？', '年度 250 點。'],
        ['從 0 點的基本估算是多少？', '以銅級一般積點率計算，未稅約 NT$7,500。'],
        ['可以把多個活動特別獲點率相乘嗎？', '不可以，套用最高的適用積點率。']
      ]
    }
  },
  campaign2x: {
  ...
  },
  {
    ...
    "sections": [
      ...
      [
        "各等級累積 100 點的估算",
        [
          "依台灣基本積點率，100 點約需：銅級 NT$3,000、銀級 NT$2,400、黃金級 NT$2,000、白金級約 NT$1,715、鑽石級 NT$1,500。",
          "每 NT$30 的積點率是銅級 1、銀級 1.25、黃金級 1.5、白金級 1.75、鑽石級 2 點。商品會逐項四捨五入，稅金不列入。"
        ]
      ],
      [
        "如何確認兌換價值",
...
:root {
    ...
    --interaction: #0f7a46;
    --interaction-hover: #0b6238;
    --border: #d8e1ee;
    --radius: 16px;
    --hero: var(--brand-dark);
}
body {
    font-family: "Noto Sans", "Noto Sans KR", "Noto Sans TC", sans-serif;
    background: var(--bg);
    min-height: 100vh;
    color: var(--text);
    line-height: 1.85;
...
.main-card {
    ...
    box-shadow: 0 18px 50px rgba(18, 42, 75, 0.09);
    overflow: hidden;
}
.hero {
    background: var(--hero);
...
.cta-btn:hover { background: var(--interaction-hover); }
...
.decision-box strong { color: var(--brand-dark); }
.number-note { font-size: 0.9rem; color: #5b6472; }
@media (max-width: 600px) {
    .main-card { margin: 0; border: 0; border-radius: 0; }
