...
const KO_ARTICLES = [
  {
    ...
    sections: [
      ...
      ['3. 일반 포인트와 캠페인 보너스를 나누기', ['2배나 3배 캠페인은 문구가 단순해 보여도 대상 앱, 결제수단, 기간, 활성화 여부, 보너스 상한이 붙을 수 있습니다. 일반 포인트는 보이는데 보너스만 안 보인다면 전체 누락이 아니라 캠페인 조건 문제일 수 있습니다.', '먼저 현재 등급의 일반 적립률로 예상치를 보고, 대상 결제에 Google Play에 표시된 최종 특별 적립률이 있다면 그 값을 따로 비교하세요. 두 숫자의 차이가 무엇이 부족한지 설명하는 데 도움이 됩니다.']],
  ...
  },
  {
    ...
    sections: [
      ...
      ['누적 포인트보다 남은 포인트가 중요합니다', ['계획을 세울 때는 목표 등급까지 남은 포인트를 입력해야 합니다. 현재 보유 포인트, 이미 사용한 포인트, 누적 포인트를 섞으면 필요한 금액이 쉽게 틀어집니다.', '계산기는 남은 포인트를 기준으로 보는 도구입니다. 현재 등급과 목표까지 부족한 포인트를 입력하고, 대상 결제에 Google Play에 표시된 최종 특별 적립률이 있다면 그 값을 비교하세요.']],
...
const TW_ARTICLES = [
  ...
  },
  {
    ...
    sections: [
      ...
      ['詢問支援前整理計算與證據', ['如果條件都符合，仍看不到加碼，請整理購買日期、活動名稱、活動名稱、一般獲點率估算、最終活動特別獲點率估算與實際顯示點數。', '訂單編號與付款資訊不要公開。計算機不能替帳號補點數，但能讓你把問題說得更清楚。']]
    ],
    faq: [
