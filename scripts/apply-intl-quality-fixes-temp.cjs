...
function replaceRequired(rel, from, to) {
  ...
  if (!before.includes(from)) throw new Error(`${rel}: required text not found: ${from}`);
...
function walk(dir, visitor) {
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) walk(abs, visitor);
    else visitor(abs);
  }
}

// 1) Korean hub wording: preserve the negation from the article title.
replaceRequired(
...
// 2) Taiwan terminology in source-of-truth scripts/runtime.
// Do not touch Hong Kong-specific sources: HK officially keeps 金級 / 鉑金級.
...
// HK is generated from the Taiwan top page. Convert Taiwan's official tier words back to HK's official terms.
replaceRequired(
  'scripts/region-page-sync.cjs',
...
// 3) Permanent Taiwan output normalization for manual/static pages.
write('scripts/tw-terminology-sync.cjs', `'use strict';
...
function syncTaiwanTerminology(rootDir) {
  ...
  if (fs.existsSync(twRoot)) walk(twRoot);
...
// 4) Explicit, semantically verified Japanese equivalents + reciprocal hreflang.
write('scripts/intl-article-hreflang-sync.cjs', `'use strict';
...
function upsertIntlJa(html, jaPath) {
  const jaTag = '    <link rel="alternate" hreflang="ja" href="' + SITE_ORIGIN + jaPath + '">';
...
// 5) Integrate both synchronizers into the canonical build.
...
// 6) Keep the international expansion sitemap aligned with HTML hreflang.
...
const {
  ...
  SITE_ORIGIN,
  intlUrl
} = require('../scripts/intl-article-hreflang-sync.cjs');
...
test('Korean article hub keeps the missing/not-completing negation', () => {
  ...
  assert.doesNotMatch(source, /퀘스트가 표시되거나 완료되지 않을 때/);
  assert.doesNotMatch(hub, /퀘스트가 표시되거나 완료되지 않을 때/);
  assert.match(source, /퀘스트가 표시되지 않거나 완료되지 않을 때/);
  assert.match(hub, /퀘스트가 표시되지 않거나 완료되지 않을 때/);
});
...
test('Taiwan pages use Taiwan official Gold and earning-rate terminology', () => {
  ...
  for (const file of files) {
    ...
    assert.doesNotMatch(html, /(?<![白黃])金級/, path.relative(root, file) + ': bare Taiwan Gold tier');
    assert.doesNotMatch(html, /累積率/, path.relative(root, file) + ': old earning-rate term');
    assert.doesNotMatch(html, /累積條件/, path.relative(root, file) + ': old earning-condition term');
  }
  const runtime = fs.readFileSync(path.join(root, 'js', 'config.js'), 'utf8');
  assert.match(runtime, /[\"']黃金級[\"']:\s*1\.5/);
  assert.doesNotMatch(runtime, /[\"']金級[\"']:\s*1\.5/);
  const seoSource = fs.readFileSync(path.join(root, 'scripts', 'intl-seo-content.cjs'), 'utf8');
  assert.doesNotMatch(seoSource, /(?<![白黃])金級/);
  assert.doesNotMatch(seoSource, /累積率/);
});

test('Hong Kong keeps its own official tier names after Taiwan terminology changes', () => {
  const hk = fs.readFileSync(path.join(root, 'hk', 'index.html'), 'utf8');
  assert.match(hk, /金級/);
  assert.match(hk, /鉑金級/);
  assert.doesNotMatch(hk, /黃金級/);
  assert.doesNotMatch(hk, /白金級/);
});
...
test('verified Japanese counterparts have reciprocal hreflang sets', () => {
  for (const [slug, jaPath] of Object.entries(ARTICLE_JA_ALTERNATES)) {
    ...
    for (const [locale, hreflang] of INTL_LOCALES) {
      ...
      assert.match(intlHtml, new RegExp('hreflang="ja" href="' + SITE_ORIGIN.replace(/[.*+?^${}()|[\\]\\\\]/g, '\\$&') + jaPath.replace(/[.*+?^${}()|[\\]\\\\]/g, '\\$&') + '"'));
      const expected = intlUrl(locale, slug);
      assert.match(jaHtml, new RegExp('hreflang="' + hreflang + '" href="' + expected.replace(/[.*+?^${}()|[\\]\\\\]/g, '\\$&') + '"'));
    }
    assert.match(jaHtml, new RegExp('hreflang="x-default" href="' + intlUrl('en', slug).replace(/[.*+?^${}()|[\\]\\\\]/g, '\\$&') + '"'));
  }
});
...
test('international expansion sitemap adds ja only for verified equivalents', () => {
  ...
  for (const [slug, jaPath] of Object.entries(ARTICLE_JA_ALTERNATES)) {
    ...
    if (!xml.includes('/en/articles/' + slug)) continue;
    const block = xml.match(new RegExp('<url>\\s*<loc>https://playpoint-sim\\.com/en/articles/' + slug.replace(/[.*+?^${}()|[\\]\\\\]/g, '\\$&') + '</loc>[\\s\\S]*?</url>'))?.[0] || '';
    assert.match(block, new RegExp('hreflang="ja" href="https://playpoint-sim\\.com' + jaPath.replace(/[.*+?^${}()|[\\]\\\\]/g, '\\$&') + '"'));
  }
  const unmapped = xml.match(/<url>\s*<loc>https:\/\/playpoint-sim\.com\/en\/articles\/google-play-points-500-1000-cost\.html<\/loc>[\s\S]*?<\/url>/)?.[0] || '';
  assert.ok(unmapped);
  assert.doesNotMatch(unmapped, /hreflang="ja"/);
});
`);
