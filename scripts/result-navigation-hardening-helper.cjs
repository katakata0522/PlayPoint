...
function replaceOnce(file, before, after) {
  ...
  if (first < 0) throw new Error(`Pattern not found in ${file}: ${before.slice(0, 120)}`);
  if (current.indexOf(before, first + before.length) >= 0) throw new Error(`Pattern is not unique in ${file}`);
...
function appendOnce(file, marker, addition) {
  const current = fs.readFileSync(file, 'utf8');
  if (current.includes(marker)) throw new Error(`Marker already exists in ${file}: ${marker}`);
