...
color: var(--reading-muted, #475569);
}
...
.comparison-table td {
padding: 12px 16px;
border: 1px solid var(--reading-border, #e2e8f0);
text-align: left;
}
...
border-color: var(--reading-border, #94a3b8);
...
function synchronizeArticle(html, localeKey, relativePath, relatedArticles = null) {
  ...
  const mainHtml = insertIntlArticlePrompt(unwrapped.slice(main.start, main.end), localeKey, { relativePath });
