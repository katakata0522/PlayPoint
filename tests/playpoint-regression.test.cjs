...
test('期限・返金・家族共有ガイドは4言語の相互参照と公式根拠を持つ', () => {
  const topics = [
    ['expiration', 'articles/2025-12-25-expiration.html'],
    ['refund', 'articles/2025-12-25-refund.html'],
    ['family-sharing', 'articles/2025-12-25-family-sharing.html']
  ];
  const locales = [
    ['en', 'GENIE.CountryCode%3DUS'],
    ['ko', 'GENIE.CountryCode%3DKR'],
    ['tw', 'GENIE.CountryCode%3DTW']
  ];
  const sitemap = fs.readFileSync(path.join(root, 'sitemap.xml'), 'utf8');

  for (const [topic, japaneseFile] of topics) {
    const japanese = fs.readFileSync(path.join(root, japaneseFile), 'utf8');
    assert.ok(japanese.includes(`/en/articles/google-play-points-${topic}.html`), japaneseFile + ' の英語hreflangがありません');
    assert.ok(japanese.includes(`/ko/articles/google-play-points-${topic}.html`), japaneseFile + ' の韓国語hreflangがありません');
    assert.ok(japanese.includes(`/tw/articles/google-play-points-${topic}.html`), japaneseFile + ' の繁体字hreflangがありません');

    for (const [locale, country] of locales) {
      const file = `${locale}/articles/google-play-points-${topic}.html`;
      const canonical = `https://playpoint-sim.com/${file}`;
      const html = fs.readFileSync(path.join(root, file), 'utf8');
      assert.ok(html.includes(`<link rel="canonical" href="${canonical}">`), file + ' のcanonicalがありません');
      assert.ok(html.includes('hreflang="ja"') && html.includes('hreflang="en"') && html.includes('hreflang="ko"') && html.includes('hreflang="zh-TW"'), file + ' のhreflangが不足しています');
      assert.ok(html.includes('"@type":"FAQPage"'), file + ' のFAQ構造化データがありません');
  ...
  for (const locale of ['en', 'ko', 'tw']) {
    const hub = fs.readFileSync(path.join(root, locale, 'articles/index.html'), 'utf8');
    for (const topic of ['cash-conversion', 'expiration', 'refund', 'family-sharing']) {
      assert.ok(hub.includes(`/${locale}/articles/google-play-points-${topic}.html`), locale + ' 記事一覧に ' + topic + ' がありません');
    }
  }
});
