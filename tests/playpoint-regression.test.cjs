...
test('Service Workerは共有URL機能を含む最新版アセットを事前キャッシュする', () => {
  ...
  assert.ok(sw.includes('playpoint-calc-v20260619'));
...
test('ブログと記事の変更アセットはバージョン付きURLで配信する', () => {
  ...
  assert.ok(blogHtml.includes(`components.js?v=${version}`));
  assert.ok(blogHtml.includes(`script.js?v=${version}`));
  assert.ok(sw.includes("playpoint-calc-v20260619"));
  for (const file of articleFiles) {
    const html = fs.readFileSync(path.join(root, 'articles', file), 'utf8');
    assert.ok(html.includes(`article-shared.css?v=${version}`), `${file} does not load versioned shared article styles`);
    assert.ok(html.includes(`../blog/article.js?v=${version}`), `${file} does not version article.js`);
    assert.ok(html.includes(`../blog/components.js?v=${version}`), `${file} does not version components.js`);
  }
});
