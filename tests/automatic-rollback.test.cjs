...
test('復旧Chromiumは本番変更前に共通helperで準備したbrowser runtimeを再利用し追加installしない', () => {
  ...
  assert.match(installBlock, /bash \.github\/scripts\/setup-browser-runtime\.sh/);
  assert.equal((workflow.match(/setup-browser-runtime\.sh/g) || []).length, 1, 'deploy must prepare browser runtime only once');
