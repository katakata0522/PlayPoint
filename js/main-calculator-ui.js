...
const CALCULATOR_FIRST_VIEW_STYLE_ID = 'calculator-first-view-style';
const CALCULATOR_FIRST_VIEW_CSS = `
...
function ensureCalculatorFirstViewStyle() {
    if (typeof document === 'undefined' || !document.head || typeof document.createElement !== 'function') return;
    if (document.getElementById(CALCULATOR_FIRST_VIEW_STYLE_ID)) return;

    const style = document.createElement('style');
    style.id = CALCULATOR_FIRST_VIEW_STYLE_ID;
    style.textContent = CALCULATOR_FIRST_VIEW_CSS;
    document.head.appendChild(style);
}

ensureCalculatorFirstViewStyle();
