...
この節以下には監査開始時点の履歴を残す。**現在の正本集計は冒頭の930/930完了であり、以下の80/850・943ケース/171ファイル等は当時値**として読む。現行回帰集合は958ケース / 173ファイル、圧縮後配信境界は17ケースで、基準930 identityとは別集計とする。件数そのものを品質目標や固定閾値にはしない。
