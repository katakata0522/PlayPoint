...
- 30分ごとのLIVE、3時間ごとのINTRADAY、6時間ごとのSearch速報は、正常な開始/終了だけなら実行ログへ残さず、健康状態シートを正本にする。WARN/ERRORと意味のある更新ログは残す。
- INTRADAYの返却メッセージは、存在しない `provisionalState` ではなく、その実行のsource状態から計算した `snapshotState` を使う。データ書込後にReferenceErrorで実行だけ失敗する状態を防ぐ。
