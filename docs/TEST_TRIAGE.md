...
**基準930中764精査・166未精査。** 第21回の国際獲得率意味13ケースが先にmainへ入ったため、753から今回11を加算する。公開HTML/CSS/JS・記事本文・地域別価格・公式用語・生成結果は変更しない。
...
## 第29回: Play Points丸めガイド13ケース（2026-09-18）

### 集計

基準930コミットに存在した `play-points-rounding-guide.test.cjs` 13ケースを、丸め実装・記事本文・記事固有UI・FAQPage・記事台帳と突合して個別精査した。既精査890へ13を加え、**903精査・27未精査**とする。ケース数・テスト内容は変更せず、現行実行集合959ケースを維持する。

### 判断

- 計算4ケースは全維持。商品ごとの四捨五入、合算との差、0未満/不正回数・不正率拒否は実behavior。
- 記事の税抜・四捨五入・獲得率/交換価値分離は利用者向け意味契約として維持。
- 40円×2回・Gold 1.5の初期例は、分割購入と合算の差を初期表示だけで再現できる意図的fixtureなので維持。
- 記事固有CTA/関連記事の各1件は、過去に発生した自動導線との重複回帰を直接防ぐため維持。
- mobile table/card・主要操作幅の検査は当該記事固有CSSの責務で、他の一般Article Design System検査だけでは同等保証にならないため維持。
- `aria-live=off` → 計算後 `role=status` の境界は初期読み上げ過多を防ぎ、操作後だけ結果を通知するアクセシビリティ契約として維持。
- knowledge boundary、visible FAQ↔FAQPage、記事台帳modified/description同期も別責務であり統合しない。

今回はテスト削除・統合・本番コード変更ともに不要と判断した。細かく見える値やDOM契約も、この専用記事で再現した過去の不具合・ユーザー向け挙動を直接守っており、単なる実装snapshotではない。

公開HTML/CSS/JS・記事本文・計算式・1728例・デザイン・生成物は変更していない。docsのみの変更である。


## 個別監査・修正の第30回（2026-09-18）
...
| 対象 | 基準ケース | 判断 |
|---|---:|---|
| `full-integrity-audit.test.cjs` | 14 | 全件維持。ゲームsource/aria、affiliate誇張防止、法務日付、CSP、収益browser、Q&A、2pt金額、表示破損、LP冪等、記事共通導線を維持。国際2x/3xのgenerator source重複2件だけ公開成果物の意味契約へ変更 |

### 今回の過剰固定・重複整理

- 国際2x/3xの獲得率意味は第21回の専用ownerで既に精査済みのため、full-integrity側で`intl-seo-content.cjs`の内部copyや`secondaryHref`記述を二重固定しない。
- EN/KO/TWの公開2x/3x LPが、最終特別獲得率を明示し、ランク通常率へ2/3を掛けないことを実成果物で検証する。
- 2xから3xへの比較導線もgenerator sourceではなく公開hrefで確認する。
...
第30回終了時の「917精査・13未精査」は、Wave18以降の国際系で**ファイル単位の未記録ケースを精査済みへ含めていた集計誤り**があることを、基準コミット `cdf5e2999719edf8e96cafeeca3a205cd9364fae` の `tests/*.test.cjs` 全168ファイルと、初期80件＋第5〜30回の明示精査記録を再照合して確認した。

基準runnerは当時から `tests/*.test.cjs` を全件自動列挙していたため、「ファイルは存在したが基準930には含まれない」という扱いにはできない。未記録9ファイルの実行時56ケースと、第22回で明示保留した2ケースを合わせ、**第30回終了時の正しい集計を872精査・58未精査へ訂正する。**

### 基準SHAの再訂正

`d46527f...` はDeploy run 35238064252で全回帰934/934の時点であり、930基準ではなかった。930件の実行正本は `cdf5e2999719edf8e96cafeeca3a205cd9364fae`（Deploy run 35217972469: 930/930）。基準後追加の `markup-contract-fixtures` 2件を現行未精査から除外し、基準930に実在する `ci-performance-sampling` 9件を未精査へ戻すため、差分は +9 -2 = 7件。第30回終了時を872/58へ訂正する。
...
**訂正後872精査から11件を加え、基準930中883精査・47未精査。** 現行テストケース数は変更せず959ケースを維持する。
...
**基準930中892精査・38未精査。** 9ケースはすべて保証を維持し、テストコード変更は不要。現行959ケースを維持する。
...
## 2026-09-26 記事追加・発見導線

OGP検査の固定記事数64を廃止し、実在する全記事について一意性・画像・メタタグを検査する。検索は既存のblog-listing-uxで略称・未掲載ゲームの計算機案内を確認する。表示は既存のarticle-design-smokeに変更記事を加え、1280/390/320pxと人気記事本文の幅を確認する。

ゲーム計算機の検索候補JSONは実行時に読み込む入口として内部参照監査へ登録する。生成元・再実行時の書き込みは既存のbuild-io-boundaries、リンク先の実在性と検索結果はblog-listing-uxで検査する。

本番検証で計算機索引の非同期到着による記事カードの差し替えを検出したため、browser-reading-uiで索引の応答を保留し、ゲーム選択後の到着でもカードが保持されることを確認する。遅延を固定時間で再現せず、案内リンクの表示を待って検査する。

## 2026-09-26 スマホの読書メニュー

browser-reading-uiを操作の主担当とし、メニュー開閉・フォーカス・幅変更時の復元・目次移動・保存リストへの往復と、JavaScript有効の全画面スクロールを確認する。目的リンクの常時表示だけを前提にした検査を、スマホでメニューから同じ行き先を開ける検査に置き換える。mobile-region-layout-smokeは日本語トップのメニューを開いて既存の地域切替を確認する。静的フォールバックの設計検査は維持する。
