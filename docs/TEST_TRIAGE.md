...
## 個別監査・修正の第29回（2026-09-18）

基準930ケースに含まれる「Play Points丸めガイド・full integrity」2ファイル27ケースを、現行Google公式のポイント計算説明、公開記事、国際campaign成果物、法務/CSP/収益browser ownerと突合して個別精査した。

**基準930中917精査・13未精査。** 27ケースはすべて保証を維持し、ケース数は増減しない。現行959ケースを維持する。公開記事・計算ロジック・campaign生成物・CSPは変更しない。

| 対象 | 基準ケース | 判断 |
|---|---:|---|
| `play-points-rounding-guide.test.cjs` | 13 | 全件維持・変更なし。公式Silver 500円→6pt、税抜対象価格、最も近い整数への丸め、購入単位丸め差、invalid input、記事限界、FAQ/台帳同期はいずれも必要 |
| `full-integrity-audit.test.cjs` | 14 | 全件維持。法務日付/CSP/収益browser/旧誤表示/2pt金額/affiliate断定防止等は維持。国際2x/3xのgenerator source重複2件だけ公開成果物の意味契約へ変更 |

### 現行公式との照合

- Google Play日本向け公式ヘルプは、税金を除いたアイテム価格にステータスの通常獲得率を掛け、獲得ポイントを最も近い整数へ丸めると説明している。
- Silverは100円あたり1.25ptで、500円なら6.25から6ptになる公式例が現在も掲載されている。
- したがって丸めロジック・税抜境界・Silver 500円例は緩和しない。

### 今回の過剰固定・重複整理

- 国際2x/3xの獲得率意味は第21回の専用ownerで既に精査済み。full-integrity側で`intl-seo-content.cjs`の内部文字列を再検査せず、公開EN/KO/TW LPが「最終特別獲得率」と「ランク通常率へ掛けない」意味を実際に持つことを検証する。
- 3xへの比較導線もgeneratorの`secondaryHref` source文字列ではなく、2x公開成果物の実hrefで確認する。
- 丸め記事のモバイル表/主要操作/FAQ/記事固有導線は記事固有UXとして意図的に維持し、一般Article Design Systemのownerへ無理に統合しない。

公開HTML/CSS/JS・記事本文・丸め実装・campaign生成器は変更していない。tests/docsのみの変更である。
