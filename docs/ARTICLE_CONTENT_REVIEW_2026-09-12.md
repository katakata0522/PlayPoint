...
- 全回帰テスト：617件中616件成功・失敗0・既定スキップ1。
- 全161記事のJSON-LD解析成功。広告枠は監査開始時の本番mainと一致。
- 全161記事のDOM検査：重複ID・目次のリンク先欠落・表の列数不一致・空見出しは修正後0件。
- 公開159記事の1280px・390px・320px、計477画面で本文の横はみ出し、見出し、キーボードフォーカス、関連記事導線を検証。通信停止で中断した回は再開し、全477件の成功記録を確認。
- 関連記事の重複を除いた後は、補助欄を生成・監査処理が認識するよう修正。再生成の冪等性を追加テストで確認。
- 完全preflight成功：コミットからの生成一致、構文、記事SEO・導線・Design System、全回帰121ファイル（616成功・失敗0・既定スキップ1）、圧縮後配信境界14件を確認。GitHubへのpush・PR作成・本番反映は未実施。
